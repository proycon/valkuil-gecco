...
ACTIONS = [
    Action(id="process", name="Tekstcorrectie", description="Controleer een Nederlandse tekst op spelfouten.",method="POST",
        command="python3 " + shellsafe(WEBSERVICEDIR + "/process_text.py",'"') +  " " + shellsafe(VALKUILDIR,'"') + " $PARAMETERS",
        allowanonymous=True,
        tmpdir=True,
    ...
    Action(id="process_sentence", name="Snelle zinscorrectie (geavanceerd)", description="Verwerkt één voorgetokeniseerde zin en geeft een JSON antwoord terug met suggesties voor correctie.",
        command="python3 " + shellsafe(WEBSERVICEDIR + "/process_sentence.py",'"') +  " " + shellsafe(VALKUILDIR,'"') + " $PARAMETERS",
        allowanonymous=True,
        tmpdir=True,
